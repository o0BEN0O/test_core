...
static int ipaddr_flush(void)
{
	...
	while ((max_flush_loops == 0) || (round < max_flush_loops)) {
		...
		if (filter.flushed == 0) {
 flush_done:
 			printf("%s[%d] show_stats[%d]\n",__func__,__LINE__,show_stats);
			if (show_stats) {
				if (round == 0)
...
static int ipaddr_list_flush_or_save(int argc, char **argv, int action)
{
	...
	printf("%s[%d] oneline[%d]\n",__func__,__LINE__,oneline);
	...
	while (argc > 0) {
		...
		if (strcmp(*argv, "to") == 0) {
			...
			filter.scope = scope;
			printf("%s[%d] filter.scopemask[%d] filter.scope[%d]\n",__func__,__LINE__,filter.scopemask,filter.scope);
...
static int ipaddr_modify(int cmd, int flags, int argc, char **argv)
{
	...
	while (argc > 0) {
		...
		if (strcmp(*argv, "peer") == 0 ||
			...
			//printf("addattr_l %s[%d] lcl.bytelen[%d]\n",__func__,__LINE__,lcl.bytelen);
	...
	if (req.ifa.ifa_prefixlen == 0){
		...
		printf("%s[%d] req.ifa.ifa_prefixlen[%d] \n",__func__,__LINE__,req.ifa.ifa_prefixlen);
	}
	...
	if (!scoped && cmd != RTM_DELADDR){
		...
		printf("%s[%d] req.ifa.ifa_scope[%d]\n",__func__,__LINE__,req.ifa.ifa_scope);
	}
