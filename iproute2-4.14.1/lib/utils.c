...
//	printf("%s[%d] family[%d] AF_PACKET[%d] AF_INET6[%d] AF_UNSPEC[%d]\n",__func__,__LINE__,family,AF_PACKET,AF_INET6,AF_UNSPEC);
	if (family == AF_PACKET) {
		int len;
...
int get_prefix_1(inet_prefix *dst, char *arg, int family)
{
	...
	if (err == 0) {
		...
		if (slash) {
			//printf("%s[%d]  dst->bitlen[%d]  dst->flags[%d]\n",__func__,__LINE__,dst->bitlen,dst->flags);
...
int matches(const char *cmd, const char *pattern)
{
	...
	printf("%s[%d] cmd [%s] pattern[%s]\n",__func__,__LINE__,cmd,pattern);
