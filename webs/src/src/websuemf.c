...
void emfSchedProcess()
{
	...
	int 	c = 0;
	const 	int	MAX_COUNT_FOR_SCHED = 200; 
 ...
 */
	...
	schedid = next;
	for (c = 0; c < MAX_COUNT_FOR_SCHED; c++) {

		JRD_OEM_LOG_INFO(JRD_OEM_LOG_HIGH, "emfSchedProcess proc:%d %d %d\n", schedid, schedMax, next);

		if (schedMax <= 0) {
			JRD_OEM_LOG_INFO(JRD_OEM_LOG_HIGH, "emfSchedProcess return:%d %d %d\n", schedid, schedMax, next);
			return;
		}
		
		if (sched != NULL && ((s = sched[schedid]) != NULL) &&	(int)s->at <= (int)time(0)) {
			
			JRD_OEM_LOG_INFO(JRD_OEM_LOG_HIGH, "TimerProc start : %d %d %d\n", schedid, schedMax, next);
			TimerProc(schedid);
			JRD_OEM_LOG_INFO(JRD_OEM_LOG_HIGH, "TimerProc end : %d %d %d\n", schedid, schedMax, next);
 ...
 */
 			JRD_OEM_LOG_INFO(JRD_OEM_LOG_HIGH, "emfSchedProcess proc:%d end\n", schedid);
			return;
		}
	}
}
