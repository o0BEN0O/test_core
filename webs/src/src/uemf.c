...
long long jrd_get_timeMS(void)
{
	struct timeval tv;	
	gettimeofday (&tv , 0);

	long long secnow = tv.tv_sec;
	long long usec = tv.tv_usec;

	long long sp = secnow * 1000 + usec/1000;
	return sp;
}
...
int jrd_wifi_save_log(char *msg)
{
	int ac = 0;
	static int swritecounter = 0;

	const int  	MAXWRITE = 10 * 10000;    //about 10MBytes
	const char 	*WEBSLOGFLAG = "/tmp/log/webslog_flag";
	const char  *WEBSLOGFILE = "/tmp/log/webs.log";


	ac = access(WEBSLOGFLAG,  F_OK);
	if (ac != 0)
	{
		return 0;
	}
	...
	FILE *fp = fopen(WEBSLOGFILE, "a+");
	if (fp)
	{
		int w = 0;
		char sztime[64];
		long long timenow = jrd_get_timeMS();
		sprintf(sztime, "[%lld]", timenow);
		
		w = fwrite(sztime, 1, strlen(sztime), fp);
		w = fwrite(msg, 1, strlen(msg), fp);
		
		fclose(fp);
	}

	swritecounter++;
	if (swritecounter > MAXWRITE)
	{
		swritecounter = 0;
		remove(WEBSLOGFILE);
	}
...
int jrd_wifi_printto_kmsg(char *msg)
{
	...
	jrd_wifi_save_log(msg);

	return 0;
}


void jrd_trim(char *t) 
{			
	int len = strlen(t);	
	int i = 0;	
	int s = 0;
