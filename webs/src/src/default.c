...
#else
#define websBuildWhitelist(void)
                       ...
                       char_t *url, char_t *path, char_t *query)
{
  ...
  if (g_webs_data.webs_unconnected_redirect == 1)
  {
    ...
    if( TRUE == wp->b_redirect )
    {
      ...
      tmp = websGetHost();
      if(strlen(tmp))
        sprintf(redirect_page, "http://%s/", tmp);
      else
        sprintf(redirect_page, "http://%s/", wp->ifaddr);

      strcat(redirect_page, websDefaultPage);

      JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "JRD unconnected redirect page: %s\n", redirect_page);
      ...
      if (wp->b_default_unredirect == TRUE)
      {
        wp->b_default_unredirect = FALSE;
        return 1;
      }
#endif
    ...
    if( TRUE == wp->b_redirect )
    {
      ...
      memcpy(redirect_page_temp, JRD_REDIRECT_PAGE, sizeof(JRD_REDIRECT_PAGE));
      sprintf(redirect_page, "http://%s", redirect_page_temp);
      JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "JRD connected redirect page: %s\n", redirect_page);

      if( is_redirect_page == TRUE )
      {
        JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "wp->b_redirect = FALSE\n");
        wp->b_redirect = FALSE;
      }
                   ...
                   0666) < 0)
  {
    ...
    //websError(wp, 404, T("Cannot open URL"));modify by houailing:2012-08-28
  ...
  /*******************modify by houailing:2012-11-07*******************************/
  if(1 == wp->isDownloadfile)
  {
...
#ifdef JRD_RO_WEBRC
  int iscfgak = 0;
# define JRD_WEBRC_CFGBAK T("/jrd-resource/resource/webs-cfg")
# define JRD_CFGBAK T("/cfgbak")
...
#ifdef JRD_RO_WEBRC
    //JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "path = %s\n",path);
    if( NULL != gstrstr(path, JRD_CFGBAK) )
    {
      gstrcpy(lpath, JRD_WEBRC_CFGBAK);
    }
    else
#endif
    {
