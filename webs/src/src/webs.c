...
#define JRD_STATIC_SECURITY_STRING "dGNsX2FsY2F0ZWxfaGVhZGVyX21ldGE="
#define JRD_STATIC_SECURITY_STRING_OPPO "=EGdl12XyVGZhVGafxWZ0F2YsF2XsNGd"
...
#define JRD_SECURITY_NUMBER 80
#define JRD_SECURITY_NUMBER_LEN 40
char jrd_security_path[JRD_SECURITY_NUMBER][JRD_SECURITY_NUMBER_LEN] = {
       "/goform/setPassword",            //0
       "/goform/setLogout",                     
       "/goform/setUsernameAndPW",
       "/goform/changePIN",
       "/goform/switchPIN",
       "/goform/setWanConnect",
       "/goform/setWanConnectMode",
       "/goform/setNetworkSelect",
       "/goform/setNetworkRegister",
       "/goform/setNetworkInfo",              
       "/goform/setWlanInfo",                 //10
       "/goform/setWpsPin",
       "/goform/setWpsPbc",
       "/goform/deleteSMS",
       "/goform/sendSMS",
       "/goform/saveSMS",
       "/goform/setSMSSetting",
       "/goform/setSMSAutoRedirectSetting",
       "/goform/setUseHistoryClear",
       "/goform/setDataPlan",              
       "/goform/setFirewallSwitch",         //20
       "/goform/addPortFwding",
       "/goform/deletePortFwding",
       "/goform/editPortFwding",
       "/goform/setDMZInfo",
       "/goform/addIPFilter",
       "/goform/editIPFilter",
       "/goform/deleteIPFilter",
       "/goform/setRouterInfo",
       "/goform/setMACFilterInfo",    
       "/goform/setReboot",            //30
       "/goform/setReset",
       "/goform/setBackupSettings",
       "/goform/EditProfile",
       "/goform/AddNewProfile",       
       "/goform/setProfileDefault",
       "/goform/setProfileDelete",     
       "/goform/setPasswordSaveInfo", 
       "/goform/setWanDisconnect",
       "/goform/setWanConnectCancel",   
       "/goform/SendUSSD",                   //40
       "/goform/SetUSSDEnd",
       "/goform/startCatchLog",
       "/goform/stopCatchLog",
       "/goform/saveLogConfig",
       "/goform/setQXDMConfigType",
       "/goform/deleteSMSReport",
       "/goform/setEmailCaptureEnable",
       "/goform/setDeviceChangeSIM",
       "/goform/setWifiSwitch",     
       "/goform/setDebugMode",              //50
       "/goform/setFOTAStartCheckVersion",
       "/goform/setFOTAStartDownload",
       "/goform/setFOTACancelDownload",
       "/goform/setFOTAStartUpdate",
       "/goform/ editPhoneBook",
       "/goform/setProfileSave",
       "/goform/setWanDisconnectCancel",
       "/goform/SetUpnpSettings",          
       "/goform/addPhoneBook",                 
       "/goform/deletePhoneBook",            //60
       "/goform/SetConnectedDeviceBlock",
       "/goform/SetDeviceUnlock",
       "/goform/SetUsageSettings",
       "/goform/SetWlanOff",
       "/goform/SetWlanOn",
       "/goform/setRouterSetting",                  
       "/goform/SetPowerSavingMode",
       "/goform/setSDShareState",
       "/goform/deleteFile",                      
       "/goform/setUploadFolder",          //70
       "/goform/newFolder",
       "/goform/deleteAllPhoneBooks",
       "/goform/setSMSRead",
       "/goform/setPowerSaveTime",
       "/goform/setWifiPerformance",
       "/goform/setUSSDSend",
       "/goform/setUpdateSettings",
       "/goform/setActiveSIM",
};

#define JRD_SECURITY_NUMBER_NO_LOGIN 8
char jrd_security_path_no_login[JRD_SECURITY_NUMBER_NO_LOGIN][JRD_SECURITY_NUMBER_LEN] = {
       "/goform/setLogin",
       "/goform/unlockPIN",
       "/goform/unlockPUK",
       "/goform/setPINAutoValidate",    //4
       "/goform/unlockSIMLock",
       "/goform/setNewPageFlagSemaphore",
       "/goform/setLanguage",
};
...
static int 		websGetInput(webs_t wp, char_t **ptext, int *nbytes, int type );
 ...
 */
	for (i = 0; i <= retries; i++) {
		...
		websListenSock = socketOpenConnection(websIpaddr, port, websAccept, 0, FALSE);
		if (websListenSock >= 0) {
			break;
		}
		port++;
...
#define POST_SDSHARE_FINSHED 1
#define POST_SDSHARE 2
#define UN_POST_SDSHARE -1
#define JRD_WEB_SDCARD_FILE T("uploadSdcardFile")
// define the buf we wirte tu file for 16k
#define	WEB_SDSHARE_SOCKET_BUFSIZ 16384
extern int sdshareFileFormat(char_t *req_name, char_t *file_name);

int	socketReadSdshareFile(int sid, char *buf, int bufsize)
{
	socket_t	*sp;
	char        readBuf[WEB_SDSHARE_SOCKET_BUFSIZ];
	...
	room = min(sizeof(readBuf) , bufsize);
	...
	for (bytesRead = 0; bufsize > 0; ) {
		 ...
		 */
		room = min(room, bufsize);

		len = socketGetInput(sid, readBuf, room, &errCode);
		st_len += len;
		JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "re read len:%d, st_len:%d, room:%d, sid:%d\n", len, st_len, room, sid);
		JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "re read sp->flags:0x%06x, errCode:%d, errCode:%06x\n", sp->flags, errCode, errCode);
		if (len < 0) {
		...
		memcpy(&buf[bytesRead], readBuf, len);
		bufsize -= len;
		bytesRead += len;
	}
	return bytesRead;
}
int jrd_write_sdshare_file(const char* file_path_name,const char *data, const int data_len)
{
  int loc_write, numLeft, numWrite;
  FILE *       fp;
  if ((fp = fopen(file_path_name, "a+")) == NULL)
  {
...
static int GetSdshareFileName(char * file_name, char *buf)
{
	char * p1 = strstr(buf, T("filename"));//"filename");
	char *p2 = NULL;
	int len = 0;
	if(p1 == NULL)
		return -1;
	...
	memcpy(&(file_name[len]), p1, p2-p1);
	file_name[len+p2-p1] = 0;
	JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "file_name:%s\n", file_name);

	return 0;
}
static int websPostHandlerRequestSdshare(webs_t wp, char_t *text, int nbytes)
{ 
	int		 size;
	int len_ct_type;
	/*
	 * supported for sdshared upload file
	 */
//	char  file_name[128] = "/media/card/";
	char  file_name[128] = "/tmp/sdshareUploadDir/";
	char cmd[156];
	char* req_name = gstrchr(&(wp->path[1]), '/');
	char* ptext = NULL;
	char* pHeadSumText = NULL;
	int   len_HeadSum= 0;
	if(!req_name)
	{
		JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "wp-path:%s\n", wp->path);
		return UN_POST_SDSHARE;
	}
	req_name++;

	if(0 != strncmp(req_name, JRD_WEB_SDCARD_FILE, sizeof(JRD_WEB_SDCARD_FILE)))
	{
		return UN_POST_SDSHARE;
	}
	JRD_OEM_LOG_INFO(JRD_OEM_LOG_MEDIAM, "nbytes:%d, clen:%d\n", nbytes, wp->clen);
	...
	if (text != NULL)
	{
		char buf[WEB_SDSHARE_SOCKET_BUFSIZ +1];
		int nnbytes= 0;
		int len = 0;
		int rq_len = 0;
		ringq_t		*rq;
		...
		JRD_OEM_LOG_INFO(JRD_OEM_LOG_MEDIAM, "req_name is %s, file_name:%s\n",req_name, file_name);
		rq = &sp->inBuf;

		//write the data from rq to sdshare file

		wp->flags |= WEBS_POST_DATA;
		/*
		 * get the file name and remove http header
		 */
#define CONTENT_TYPE_STRING "Content-Type:"
#define CONTENT_TYPE_END_STRING "\r\n\r\n"
/*
 * caculate the http sum length
 */ 
		ptext = strstr(text, "\r\n");

		len_HeadSum = ptext -text;

		/*
		 *
		 * remove http head
		 */
		memcpy(buf, text, nbytes);//nbytes - (ptext - text+len_ct_type));
		wp->clen -= nbytes;
		//rq_len = 0;//ringqLen(rq);
		JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "nbytes:%d\n", nbytes);
		if (wp->clen > 0) {
			rq_len = ringqLen(rq);
			memcpy(&(buf[nbytes]), rq->servp, rq_len);
			//	if(nnbytes >len_HeadSum)
		}

		// remove the http post head 
		ptext = strstr(buf, CONTENT_TYPE_STRING);
		if(ptext == NULL)
		{
			JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "Content-Type: application/octet-stream Couldn't Find\n");
			return UN_POST_SDSHARE;
		}
		ptext = strstr(ptext, CONTENT_TYPE_END_STRING);
		if(ptext == NULL)
		{
			JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "Content-Type: end Couldn't Find\n");
			return UN_POST_SDSHARE;
		}

		len_ct_type = strlen(CONTENT_TYPE_END_STRING);

		//set upload file_name 
		if(0 != GetSdshareFileName(file_name, buf))
			strcat(file_name,"byh_empty_fn");

		sprintf(cmd, "rm -f %s", file_name);
		system(cmd);
		//when the file finished upload, we should remove the http head and tail
		//otherwith remove http head
		if(rq_len == wp->clen)
		{
			memcpy(&buf[bytes], rq->servp, rq_len);
			jrd_write_sdshare_file(file_name, &(buf[ptext - buf+len_ct_type]), rq_len + nbytes - (ptext - buf+len_ct_type) - len_HeadSum);
		}
		else
		{
			jrd_write_sdshare_file(file_name, &(buf[ptext - buf+len_ct_type]), rq_len + nbytes - (ptext - buf+len_ct_type));
		}
		// end remove http post head

		ringqGetBlkAdj(rq, rq_len);
		wp->clen -= rq_len;
		if (wp->clen <= 0) {
			goto SDSHARE_END_RECIVE;
		}

		//recive the fill untill finish 
		//		file size bigger than 1046 
		do{
			if(nnbytes == wp->clen)
			{
				//bigger file
				JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "calloc len_HeadSum:%d\n", len_HeadSum);
				if(nnbytes >len_HeadSum)
					jrd_write_sdshare_file(file_name, buf, nnbytes - (len_HeadSum + 6));
				break;
			}

			if(nnbytes> 0)
				jrd_write_sdshare_file(file_name, buf, nnbytes);
			wp->flags |= WEBS_POST_DATA;
			wp->clen -= nnbytes;
			if (wp->clen <= 0) {
				break;
			}
			len = (wp->clen > WEB_SDSHARE_SOCKET_BUFSIZ) ? WEB_SDSHARE_SOCKET_BUFSIZ: wp->clen;

			nnbytes= socketReadSdshareFile(wp->sid, buf, len);
			JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "nnbytes :%d, wp->clen:%d\n", nnbytes, wp->clen);
		}while(1);
	}
SDSHARE_END_RECIVE:
	JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "req_name:%s, file_name:%s\n", req_name, file_name);
	//	req_name= "/usr/oem/byh_result";

//	sdshareFileFormat(req_name, file_name);

	return POST_SDSHARE_FINSHED;
}
#endif
...
{
	char_t 	*text;
	int		rc, nbytes, len, done, fd, size, ret;
	char_t		*req_name; 
 ...
 */
		while ((rc = websGetInput(wp, &text, &nbytes, ret)) == 0) {
			;
		}
...
#endif
			{
#if JRD_FEATURE_SDSHARE
				ret = websPostHandlerRequestSdshare(wp, text, nbytes);
				if (ret == POST_SDSHARE_FINSHED)
				{
					goto JRD_SDSHARE_END;
				}
				else if(ret == POST_SDSHARE)
				{
					if (nbytes > 0) {
						break;
					}
               ...
               //add by lichangqin for upload file bigger then 1M bytes 2014-8-5 begin
#ifdef JRD_UPLOAD_LARGE_FILE
				int maxlen = (wp->lenPostData + nbytes + 1) * sizeof(char_t);
					if(maxlen > 1024*1024)
					{
					   JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "more bigger file.\n");
					   	if (fd != -1) {
							gclose (fd);
							fd = -1;
						}
...
#endif
				//add by lichangqin for upload file bigger then 1M bytes 2014-8-5 end
...
#define JRD_HOST_KEY "Host:"
#define JRD_HOST_KEY_LEN 5

static int rember_url_flag;
static char host_url[128];
...
#define   LAST_WEB_FOR_REDIRECT   "/jrd-resource/resource/last_web_for_redirect"
#define JRD_USER_AGENT_KEY                 "User-Agent:"
#define JRD_USER_AGENT_KEY_LEN        11

static int jrd_oem_remeber_user_agent_and_url(char * user_agent_r, char *host_url_r)
{
  FILE *f;
  char buf[512]={0};
  int has_usr_agent=0;
  ...
  //printf("now remeber user_agent_r=%s, host_url_r=%s\n ", user_agent_r, host_url_r);
 //inquiry  user_agent_r in the file
  f = fopen(LAST_WEB_FOR_REDIRECT, "r");
  if (f==NULL)
  {
   JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "jrd oem fopen last_web_for_redirect fail1\n");
   return -1;
  }  

  while (fgets(buf, sizeof(buf), f)) 
  {
	if (NULL != strstr(buf, user_agent_r)) 
	{
	    has_usr_agent = 1;
        break;
	 }  
   ...
   //printf("redirect has_usr_agent=%d\n",has_usr_agent);

    if(0 == has_usr_agent)  //if user_agent_r is't in the file, add it in the file
    {
      f = fopen(LAST_WEB_FOR_REDIRECT, "a+");
      if (f == NULL)
      {
       JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "jrd oem fopen last_web_for_redirect fail2\n");
       return -1;
      }
    ...
    else                            //if user_agent_r is in the file, change it in the file
   {

      f = fopen(LAST_WEB_FOR_REDIRECT, "r");
      if (f==NULL)
      {
       JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "jrd oem fopen last_web_for_redirect fail3\n");
       return -1;
      }
      ...
      *f2=0;
      ...
      while (fgets(buf, sizeof(buf), f)) 
      {
    	if (NULL != strstr(buf, user_agent_r))   continue;
      
        strcat(f2, buf);      
       }
        fclose(f);

        f = fopen(LAST_WEB_FOR_REDIRECT, "w");
        if (f==NULL)
       {
         JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "jrd oem fopen last_web_for_redirect fail4\n");
         return -1;
       }
        fputs(f2,f);
        fprintf(f, "%s=%s\n", user_agent_r, host_url_r);
...
static int websGetInput(webs_t wp, char_t **ptext, int *pnbytes, int type) 
{
	char_t	*text;
	...
	char_t	*str_tmp =  NULL;
	int host_len = 0;
    char_t *user_agent_tmp=NULL;
    int i = 0;
    char_t	*str_dn_tmp =  NULL;
    char_t filter_dn[][32]=
    {
 //    "wpad",
      "msftncsi.com",
	  ...
	  FILE *stream = NULL;
	  static char_t is_executed_redirect = FALSE;
	  char_t is_redirect_page;
#endif
       int path_number =0;
 ...
 */
	...
	//JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "len:%d, wp->flags:0x%06x\n", len, wp->flags);
	...
	if (len > 0) {
		...
		if (nbytes < 0) {						/* Error */
			websDone(wp, 0);
			JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "nbytes:%d\n", nbytes);
			return -1;
 ...
 */
    ...
    if(nbytes > E_MAX_REQUEST)
    {
      ...
      {
        websError(wp, 404, T("HEADER invalid"));
        return -1;
      }
    }
		if (nbytes < 0) {
 ...
 */
		...
		else if (g_webs_data.webs_unconnected_redirect == 1){
		if(wp->flags & WEBS_GET_REQUEST)
		{
		  if(!strnicmp(text, JRD_HOST_KEY, JRD_HOST_KEY_LEN))
		  {    

          #ifdef JRD_UNCONNECT_REDIRECT_WEBHOST
		    host_len = strlen(websHost);
		    str_tmp = text + JRD_HOST_KEY_LEN + 1;
			
		    //JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "unconnected filter_dn : text{%s} local_host{%s}, wp->ifaddr{%s}, str_tmp{%s}\n", \
		    //text, websHost, wp->ifaddr, str_tmp);
		    ...
		    if(host_len)
		    {
		      ...
		      {
		   #if 1
                 for(i=0; i<sizeof(filter_dn)/sizeof(filter_dn[0]); i++)
		        {
		        JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW,"filter_dn : str_tmp{%s}\n", str_tmp);
		          if( NULL != strstr(str_tmp, filter_dn[i]) )
		          {
		           JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW,"filter_dn : filter dn{%s}\n", filter_dn[i]);
		            wp->b_redirect = FALSE;
		            wp->b_default_unredirect = TRUE;
		            break;
		          }
		        }
           #endif
		      ...
		      {
		        wp->b_redirect = FALSE;
			JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "jrd unconnected str_tmp2: %s, b_redirect: %d\n", str_tmp, wp->b_redirect);
		      }
		    }
		    else
		    {
		      if(strnicmp(str_tmp, wp->ifaddr, strlen(wp->ifaddr)))
		      {
		        wp->b_redirect = TRUE;
			JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "jrd unconnected str_tmp3: %s, b_redirect: %d\n", str_tmp, wp->b_redirect);
		      }
		    }
    #ifdef JRD_UNCONNECT_REDIRECT_REMEBER_LAST_URL
          if(wp->b_redirect == TRUE)
            {
              rember_url_flag=1;
          memset(host_url, 0, sizeof(host_url));
          strncpy(host_url, str_tmp, sizeof(host_url)-1);
            }
    #endif
		  #endif	
		    ...
		    JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "filter_dn : text{%s} local_host{%s}, wp->ifaddr{%s}, str_tmp{%s}\n", \
		    text, websHost, wp->ifaddr, str_tmp);
		     ...
		     if(strnicmp(str_tmp, websHost, host_len) && \
		         strnicmp(str_tmp, wp->ifaddr, strlen(wp->ifaddr)))
		      {
		        for(i=0; i<sizeof(filter_dn)/sizeof(filter_dn[0]); i++)
		        {
		          str_dn_tmp = strstr(str_tmp, filter_dn[i]);
		          if( str_dn_tmp != NULL )
		          {
			    JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "filter_dn : filter dn{%s}\n", filter_dn);
		            wp->b_redirect = FALSE;
		      ...
		      {
		        JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "jrd is websHost\n");
		        wp->b_redirect = FALSE;
		      }
			  
		      if(!strnicmp(str_tmp, JRD_REDIRECT_PAGE, sizeof(JRD_REDIRECT_PAGE)))
		      {
    			JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "JRD has executed redirect,now is_redirect_page == TRUE\n");
    ...
    #ifdef JRD_UNCONNECT_REDIRECT_REMEBER_LAST_URL   
          if(!strnicmp(text, JRD_USER_AGENT_KEY, JRD_USER_AGENT_KEY_LEN))
          {     
           if(rember_url_flag==1)
           {
            rember_url_flag = 0;              
            user_agent_tmp = text + JRD_USER_AGENT_KEY_LEN +1;
            jrd_oem_remeber_user_agent_and_url(user_agent_tmp, host_url);              
            }
           }
   #endif
...
/****modify by houailing:2014-07-18 begin for bug739754 ***/
#ifndef JRD_WEBS_NO_CGI_TMPFILE
		if (wp->flags & WEBS_POST_REQUEST) {
			wp->cgiStdin = websGetCgiCommName();
		}
#endif
/****modify by houailing:2014-07-18 end for bug739754 ***/
	}
#endif
 ...
 */
		...
		else if(gstricmp(key, T("_TclRequestVerificationToken")) == 0)
		{
			if (value == NULL) {
				wp->token = NULL;
			}
			...
			{
				wp->token = jrd_decrypt(value);
			}

		}
		else if (gstricmp(key, T("_TclRequestVerificationKey")) == 0)
		{
			wp->cer_key = bstrdup (B_L, value);
		}
 ...
 */
         ...
         {
            wp->flags |= WEBS_CLEN;//add by houailing
            wp->clen = 0;
         }
 ...
 */
    ...
    wp->isDownloadfile = 0;//add by houailing:2012-11-07 
...
{
	a_assert(ipaddr && *ipaddr);
  JRD_OEM_LOG_INFO(JRD_OEM_LOG_MEDIAM, "ipaddr %s\n", ipaddr);  
