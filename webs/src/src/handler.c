 ...
 */
   ...
   {
      f = fopen("/jrd-resource/resource/browser_user_agent_for_redirect", "w");
      if (f==NULL)
     {
       JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "jrd oem fopen browser_user_agent_for_redirect fail\n");
       return -1;
     }
      fprintf(f, "%s", wp->userAgent);
      fclose(f);
   }
#endif
 ...
 */
  ...
  //JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "req_wp: 0x%08X\nnurl:%s\npath:%s\nquery:%s\npostdata:%s\n\n", wp, wp->url, wp->path, wp->query, wp->postData);
/*
 ...
 */
	...
	for (i = 0; i < websUrlHandlerMax; i++) {
      ...
      /*JRD add start, Li Yang 2013/10/11*/
#ifdef JRD_NEW_WEB_ARCH
      /*JRD LY comments
        Add goform filter here, except login related all should processed in JRD process function
        So should continue this loop*/
      /*JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "Handler id: %d, prefix: %s, request path: %s\n", i, sp->urlPrefix, wp->path);
      if((gstrncmp(T("/jrd/webapi"), wp->path, 11)) == 0) // jie.li modify, 2014/9/5
      {		   
        if(!jrd_form_filter(wp,wp->path))
        {
          JRD_OEM_LOG_INFO(JRD_OEM_LOG_LOW, "jrd_form_filter return false\n", i, sp->urlPrefix, wp->path);
          if(websJRDJsonHandler(wp, sp->urlPrefix, sp->webDir, sp->arg, wp->url, wp->path, wp->query))
          {
            return 1;
          }
        }
      ...
      {
        //Invoke JRD handler and send the data to oem-app
        if(websJRDJsonHandler(wp, sp->urlPrefix, sp->webDir, sp->arg, wp->url, wp->path, wp->query))
        {
          return 1;
        }
...
#endif
      /*JRD add end, Li Yang 2013/10/11*/
  		if (first) {
  			websSetEnv(wp);
  			first = 0;
  		}
