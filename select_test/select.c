...
int SetOpt(int fd,int nSpeed,int nBits,char nEvent,int nStop)
{
	...
	newtio.c_lflag &=~ICANON;//åŽŸå§‹æ¨¡å¼

	//newtio.c_lflag |=ICANON; //æ ‡å‡†æ¨¡å¼
...
int Read(int fd, u8 *readbuf, int BUFSIZE)
{
	int i=0;
  int len = 0;
  int nfds,select_ret;
  ...
  timeout.tv_sec = 1;
  timeout.tv_usec = 0;
  if((select_ret = select(nfds, &readfds, NULL, NULL, &timeout)) > 0)
  {
...
UART_RX_STATUS_EM uart_rx(void)
{
	...
	Read(fd,rx_buf,UART_DATA_BUFF_LEN);
	return 0;
...
int main (int argc, char *argv[])
{
	...
	//fd = open(DEV_NAME, O_RDWR|O_NOCTTY|O_NDELAY);//O_RDWR | O_NOCTTY | O_NDELAY | O_EXCL
	fd = open(DEV_NAME, O_RDWR | O_NOCTTY | O_NDELAY | O_EXCL);
	if(fd < 0) {
		perror(DEV_NAME);
		return -1;
	}
	...
	for(time_out=0;time_out<2;time_out++){
		...
		tcflush(fd, TCIFLUSH);		//æ¸…ç©ºè¾“å…¥ç¼“å­˜
		tcflush(fd, TCOFLUSH);		//æ¸…ç©ºè¾“å‡ºç¼“å­˜
		tcflush(fd, TCIOFLUSH); 	//æ¸…ç©ºè¾“å…¥è¾“å‡ºç¼“å­˜
		ioctl(fd, TCFLSH, 0); 	 //æ¸…ç©ºè¾“å…¥ç¼“å­˜
		ioctl(fd, TCFLSH, 1); 	 //æ¸…ç©ºè¾“å‡ºç¼“å­˜
		ioctl(fd, TCFLSH, 2); 	 //æ¸…ç©ºè¾“å…¥è¾“å‡ºç¼“å­˜
